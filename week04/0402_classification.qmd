...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=20240508)
```
...
```{python}
#| echo: false
predict_mismatch = comparison_df.groupby("correct").size().reset_index(name = "count")["count"][0]
display(Markdown("""
不一致の件数は {predict_mismatch}/{total_n}件.
""".format(predict_mismatch = predict_mismatch, total_n = len(comparison_df))))
```
