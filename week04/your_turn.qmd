...
penguins_small = penguins.groupby("sex").apply(lambda x: x.sample(frac=0.3, random_state=20240510))
penguins_small.reset_index(drop=True, inplace=True)

X = penguins_small.drop(columns="sex")
y = penguins_small["sex"]
```
...
以下の表のspecies列に「雄」だと思えば`M`（Male）、「雌」だと思えば`F`(Female)を記載します。

```{python}
correct_df = pd.concat([X_test, y_test], axis=1)
test_out = correct_df[penguins.drop(columns=["sex"]).columns]
```
